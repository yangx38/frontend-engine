...
class FiscalStaffApproveRequests extends Component {
    state = {
        searchText: '',
        searchedColumn: '',
    };
    ...
    componentDidMount() {
        ...
        const { getFormsFromFiscalStaffNetId } = this.props;
        if (login) {
            const netId = email.split('@')[0];
            getFormsFromFiscalStaffNetId(netId);
        }
    }
    ...
    getFormTable() {
        ...
        const formTableColumns = [
            ...
            },
            {
                ...
                dataIndex: '',
                key: 'x',
                render: () => <Link to={'/fiscalstaff/approverequests/requestdetails'}>Details</Link>,
            },
        ];
        return (
            <Fragment>
...
const mapStateToProps = (state) => {
    return {
        ...
        ft_allforms: state.getIn(['fiscalstaff_approverequests', 'ft_allforms']),
    }
}
...
const mapDispatchToProps = (dispatch) => {
    return {
        // componentDidMount()
        getFormsFromFiscalStaffNetId(netId) {
           dispatch(actionCreators.getFormsFromFiscalStaffNetId(netId));
        },
        // getFormTable()
...
export default connect(mapStateToProps, mapDispatchToProps)(FiscalStaffApproveRequests);