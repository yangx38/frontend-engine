...
class SubmitterMyRequests extends Component {
    ...
    componentDidMount() {
        ...
        const { getFormsFromSubmitterNetId } = this.props;
        if (login) {
            const netId = email.split('@')[0];
            getFormsFromSubmitterNetId(netId);
        }
    }
    ...
    getFormTable() {
        ...
        const { directToMyRequestDetailsPage } = this.props;
        const formTableColumns = [
            ...
            },
            {
                ...
                dataIndex: '',
                key: 'x',
                render: () => <Link to={'/submitter/myrequests/requestdetails'}>Details</Link>,
            },
        ];
        return (
            <Fragment>
                <Table className='tableCursor' columns={formTableColumns} dataSource={ft_allformsJS} size="small" pagination={{ pageSize: 50 }}
                    onRow={(columns, index) => { return { onClick: () => directToMyRequestDetailsPage(columns, index) }; }} />
            </Fragment>
        );
    }
...
const mapStateToProps = (state) => {
    return {
        ...
        ft_allforms: state.getIn(['submitter_myrequests', 'ft_allforms']),
    }
}
...
const mapDispatchToProps = (dispatch) => {
    return {
        // componentDidMount()
        getFormsFromSubmitterNetId(netId) {
            dispatch(actionCreators.getFormsFromSubmitterNetId(netId));
        },
        // getFormTable()
        directToMyRequestDetailsPage(columns, index) {
            dispatch(actionCreators.directToMyRequestDetailsPage(columns));
        }
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(SubmitterMyRequests);