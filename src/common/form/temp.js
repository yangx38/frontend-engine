...
class FormForSubmitter extends Component {
    getTravelRequestForm() {
        ...
        //     tempDataJS.push(<Option value={budgetnumber}>{budgetnumber.concat(' - ').concat(budgetname)}</Option>);
        // })
                 ...
                 {/* <Form.Item label="Budget Number">
                   <Input.Group compact>
                        ...
                        <Form.Item name={['budget', 'budgetamount']} noStyle rules={[{ required: true, message: 'Amount is required' }]} >
                            <InputNumber style={{ width: '35%' }} formatter={value => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} parser={value => value.replace(/\$\s?|(,*)/g, '')} />
                        </Form.Item>
